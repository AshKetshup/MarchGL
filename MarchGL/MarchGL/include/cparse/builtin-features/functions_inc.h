...
namespace builtin_functions {
	...
	inline packToken default_print(TokenMap scope) {
	...
	inline packToken default_sum(TokenMap scope) {
		// Get the arguments:
		TokenList list = scope["args"].asList();
	...
	inline packToken default_int(TokenMap scope) {
		packToken tok = scope["value"];
	...
	inline packToken default_str(TokenMap scope) {
	...
	inline packToken default_type(TokenMap scope) {
		packToken tok = scope["value"];
		packToken* p_type;
	...
	inline packToken default_sqrt(TokenMap scope) {
		// Get a single argument:
		double number = scope["num"].asDouble();

		return sqrt(number);
	}
	inline packToken default_sin(TokenMap scope) {
		// Get a single argument:
		double number = scope["num"].asDouble();

		return sin(number);
	}
	inline packToken default_cos(TokenMap scope) {
		// Get a single argument:
		double number = scope["num"].asDouble();

		return cos(number);
	}
	inline packToken default_tan(TokenMap scope) {
		// Get a single argument:
		double number = scope["num"].asDouble();

		return tan(number);
	}
	inline packToken default_abs(TokenMap scope) {
		// Get a single argument:
		double number = scope["num"].asDouble();
	...
	inline packToken default_pow(TokenMap scope) {
		// Get two arguments:
	...
	inline packToken default_list(TokenMap scope) {
		// Get the arguments:
		TokenList list = scope["args"].asList();
	...
	inline packToken default_map(TokenMap scope) {
		return scope["kwargs"];
	}

	/* * * * * Object inheritance tools: * * * * */

	inline packToken default_extend(TokenMap scope) {
		packToken tok = scope["value"];
	...
	inline std::string packToken_str(const TokenBase* base, uint32_t nest) {
		const Function* func;

...
namespace builtin_typeSpecificFunctions {
	...
	inline packToken map_pop(TokenMap scope) {
	...
	inline packToken map_len(TokenMap scope) {
		TokenMap map = scope.find("this")->asMap();
		return map.map().size();
	}

	inline packToken default_instanceof(TokenMap scope) {
	...
	inline packToken list_push(TokenMap scope) {
	...
	inline packToken list_pop(TokenMap scope) {
	...
	inline packToken list_len(TokenMap scope) {
		TokenList list = scope.find("this")->asList();
		return list.list().size();
	}

	inline packToken list_join(TokenMap scope) {
	...
	inline packToken string_len(TokenMap scope) {
		std::string str = scope["this"].asString();
		return static_cast<int64_t>( str.size() );
	}

	inline packToken string_lower(TokenMap scope) {
		std::string str = scope["this"].asString();
		std::string out;
		for (char c : str) {
	...
	inline packToken string_upper(TokenMap scope) {
		std::string str = scope["this"].asString();
		std::string out;
		for (char c : str) {
	...
	inline packToken string_strip(TokenMap scope) {
		std::string str = scope["this"].asString();
	...
	inline packToken string_split(TokenMap scope) {
		TokenList list;
		std::string str = scope["this"].asString();

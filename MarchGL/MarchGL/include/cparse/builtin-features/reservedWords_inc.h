...
namespace builtin_reservedWords {
	...
	inline packToken trueToken = packToken(true);
	inline packToken falseToken = packToken(false);
	inline packToken noneToken = packToken::None();

	inline void True(const char* expr, const char** rest, rpnBuilder* data) {
		data->handle_token(trueToken->clone());
	}

	inline void False(const char* expr, const char** rest, rpnBuilder* data) {
		data->handle_token(falseToken->clone());
	}

	inline void None(const char* expr, const char** rest, rpnBuilder* data) {
		data->handle_token(noneToken->clone());
	}

	inline void LineComment(const char* expr, const char** rest, rpnBuilder* data) {
		while (*expr && *expr != '\n') ++expr;
		*rest = expr;
	}

	inline void SlashStarComment(const char* expr, const char** rest, rpnBuilder* data) {
	...
	inline void KeywordOperator(const char* expr, const char** rest, rpnBuilder* data) {
	...
	inline void DotOperator(const char* expr, const char** rest, rpnBuilder* data) {
		data->handle_op(".");
